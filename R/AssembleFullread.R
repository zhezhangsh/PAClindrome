...
AssembleFullread <- function(fraw, output = getwd()) {
  ...
  len <- lapply(uid, function(mid) { # For every movie
    ...
    ind.end   <- split(as.integer(sapply(start.end, function(i) i[2])), hole.id);
    ...
    # if (!split) {
    #   ln <- as.vector(rbind(paste0('>', names(fullread)), as.vector(fullread)));
    #   writeLines(ln, paste0(output, '/', mid, '/fullread.fasta'));
    # };
    
    # writeLines(names(fullread), paste0(output, '/', mid, '/fullread.txt'));
